...
void gaussian_elimination_blocked(vector<double>& A, vector<double>& b, int n, int block_size = 32) {
    for (int k = 0; k < n; k++) {
        ...
        for (int i = k + 1; i < n; i++) {
            ...
            // Cache blocking: przetwarzam kolumny w blokach
            for (int jj = k; jj < n; jj += block_size) {
...
int main() {
    ofstream timeFile("czasy6.csv");
    ofstream gflopsFile("gflopsy6.csv");
    ...
    for (int n = 100; n <= 2000; n += 100) {
        ...
        gaussian_elimination_blocked(A, b, n, 16);
