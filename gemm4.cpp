...
// SIMD + OpenMP wersja eliminacji Gaussa
void gaussian_elimination(vector<double>& A, vector<double>& b, int n) {
    for (int k = 0; k < n; k++) {
        double pivot = get(A, k, k, n);

        #pragma omp parallel for schedule(static) // rozdzieli wiersze do wszystch dostępnych wątków na sprzęcie (tutaj 8)
        for (int i = k + 1; i < n; i++) {
...
int main() {
    ofstream timeFile("czasy4.csv");
    ofstream gflopsFile("gflopsy4.csv");
